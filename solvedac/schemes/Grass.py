...
class Grass(Repr):
    def __init__(self, data: JObject):
        self.grass: Sequence[History[date,int|Literal['frozen','repaired-incremented']]] = list(map(lambda x: History(datetime.strptime(x["date"], "%Y-%m-%d").date(), x["value"]), cast(Sequence[Any], data["grass"])))
        self.theme = cast(str|None, data.get("theme"))
